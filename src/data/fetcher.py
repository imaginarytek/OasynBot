...
def fetch_data_sync(symbol: str, event_time_str: str) -> list:
    """Synchronous wrapper to fetch data for a specific event time"""
    async def _run():
        f = ApiFetcher()
        # Parse time
        try:
        ...
        await f.exchange.load_markets()
        data = await f.fetch_window(symbol, event_dt)
        await f.exchange.close()
        return data
